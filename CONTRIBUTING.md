...
* If you or the maintainers don't respond to an issue for 30 days, the [issue may be closed](#clean-up-issues-and-prs). If you want to come back to it, reply (once, please), and we'll reopen the existing issue. Please avoid filing new issues as extensions of one you already made.

## Report an Error or Bug

If you run into an error or bug with the project:

* [Open an Issue][issues] at 
...
Note: The team is unlikely to want, or be able to accept, every single feature request that is filed. Please understand, if they need to say no, that it's nothing personal.
...
* Install [Node.js](https://nodejs.org/en/download/) directly or via [NVM](https://github.com/creationix/nvm).
...
And you should be ready to go! If there is a problem it could be our fault please [open an issue][issues]! We'll try to help as swiftly as we can.
...
* In your commit message(s), begin the first line with `docs ` or `documentation `. For example: `docs: Adding a contrib section to CONTRIBUTING.md`.
* [Open a new pull request][pulls] with your changes.
...
* Write clear, concise commit message(s)! We prefer commits messages to follow the following convention where they start with the following:
	* `Added <description>` for new features.
	* `Changed <description>` for changes in existing functionality.
	* `Deprecated <description>` for soon-to-be removed features.
	* `Removed <description>` for now removed features.
	* `Fixed <description>` for any bug fixes.
	* `Security <description>` in case of vulnerabilities. 
	* `Documentation <description>` when updating or adding documentation.
* Dependency updates, additions, or removals must be in individual commits, and the message must use the format: `<prefix> (deps): PKG@VERSION`, where `<prefix>` is any of the above changelog prefixes.
* [Open a new pull request][pulls] with your changes.
* If your PR is connected to an open issue, add a line in your PR's description that says `Fixes: #123`, where `#123` is the number of the issue you're fixing.

Once you've filed the PR:

* Barring special circumstances, maintainers will not review PRs until all checks pass (eg: Travis, CircleCI, Snyk, etc).
* One or more maintainers will use GitHub's review feature to review your PR.
* If the maintainer asks for any changes, edit your changes, push, and ask for another review. Additional tags (such as `needs-tests`) may be added depending on the review.
* If the maintainer decides to pass on your PR, they will thank you for the contribution and explain why they won't be accepting the changes. That's ok! We still really appreciate you taking the time to do it, and we don't take that lightly. 💚
* If your PR gets accepted, it will be marked as such, and merged into the `latest` branch soon after. Your contribution will be distributed to the masses next time the maintainers [tag a release](#tag-a-release)
...
* Go to the issue tracker and [filter open issues by the `support` label](https://github.com/abritinthebay/chromism/issues?q=is%3Aopen+is%3Aissue+label%3Asupport).
...
* Be kind and patient with users -- often, folks who have run into confusing things might be upset or impatient. This can be ok - the issue is important to them! Try to understand where they're coming from and if you're too uncomfortable with the tone feel free to stay away or withdraw from the issue. However if the user is outright hostile or is violating the project's [Code of Conduct](CODE_OF_CONDUCT.md) you may refer to it & alert the maintainers to resolve the conflict.
...
Issues and PRs can go stale after a while. Maybe they're abandoned. Maybe the team will just plain not have time to address them any time soon. In these cases they should be closed until they're brought up again or the interaction starts over.
...
* Go through each issue *from oldest to newest*, and close them if **all of the following are true**:
  * _not_ opened by a maintainer
  * _not_ marked as `critical`
  * _not_ marked as `starter` or `help wanted` (these might stick around for a while as they're intended to be usefully available)
  * there are no _explicit_ messages in the comments asking for it to be left open
  * _does not_ belong to a milestone

Leave a message when closing saying "Cleaning up stale issue. Please reopen or ping us if and when you're ready to resume this. See https://github.com/abritinthebay/chromism/blob/latest/CONTRIBUTING.md for more details."
...
Merges are only performed by team members with the correct permissions. If you believe a PR should be merged, and it has been well reviewed but possibly missed by the maintainers, please @mention one of the team. We will then try to deal with it promptly.

## Tag A Release

[Needs Collaborator](#join-the-project-team): Committer

Releases are only performed by team members with the correct permissions. The most important bit here is that all tests must pass and that tags must use [semver](https://semver.org). Discussion over if a change is MAJOR, MINOR, or PATCH is welcome in a features relevant PR.
...
[issues]: https://github.com/abritinthebay/chromism/issues
[pulls]: https://github.com/abritinthebay/chromism/pulls
